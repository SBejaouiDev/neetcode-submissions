class Solution:
    def majorityElement(self, nums: List[int]) -> int:
        ...
        - candidate: The candidate currently winning the battle

        if the count is zero that means the current cadidate was wiped out. Grab the current number and make it the new candidate
        set count to 1: 

        if the current number is equal to the candidate increment the count 

        if the current number does not equal the candidate decrement the count
        """
        ## [7 / 2 times] the array]
